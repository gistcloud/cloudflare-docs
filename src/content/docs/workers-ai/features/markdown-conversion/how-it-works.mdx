...
- Some elements are removed from the text, including `<header>`, `<footer>` and `<head>` elements.
...
- If a base URL was obtained from the last step, relative links in the HTML are resolved to fully qualified URLs
